...
public class PlayerService implements PlayerRepository {
    ...
    public ArrayList<Player> addPlayer(Player player) {
        player.setId(unique_id);
        team.put(unique_id, player);
        unique_id += 1;
        Collection<Player> playersCollection = team.values();
        ArrayList<Player> updatedList = new ArrayList<>(playersCollection);
        return updatedList;
    }
    ...
    public Player updatePlayer(int playerId, Player player) {
        ...
        if (player.getPlayerName() != null) {
            existingPlayer.setName(player.getPlayerName());
        }
